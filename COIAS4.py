...
class Asthunter(tk.Frame):
    ...
    def sub_window(self):
        ...
        self.astdata = [ast_xy[i] for i in
                        np.where(ast_xy[:, 1] == str(self.filenums[self.image_number] - 1))]  # NM 2021.08.10
        ...
        for i in range(len(self.astdata[0])):
            ...
            self.coord_on_canvas = self.canvas.create_rectangle(xpos - 20, ypix - ypos + 20, xpos + 20,
                                                                ypix - ypos - 20, outline='#000000', width=5)
    ...
    def mouseClickAndOutputObjectsNumber(self, event):
        ...
        for i in range(len(self.astdata[0])):
            xpos = int(float(self.astdata[0][i][2]))
            ypos = int(float(self.astdata[0][i][3]))
            if xpos - 20 < self.xp2 and xpos + 20 > self.xp2 and ypix - (ypos + 20) < self.yp2 and ypix - (
                    ypos - 20) > self.yp2:
        ...
        for asteroidNumberStr in self.matchAsteroidNamesStr:
            ...
            for num in self.writtenNumbers:
                ...
                if int(asteroidNumberStr) == num:
                    self.t0.delete("{}.0".format(rowWrittenNumbers),
                                   "{}.end +1c".format(rowWrittenNumbers))  # KS added 2021/7/25
                    self.messageBox.delete(0, tk.END)
                    self.messageBox.insert(tk.END, "message:")
                    matchFlag = 1
            if matchFlag == 0:
        ...
        for i in range(len(self.astdata[0])):
            ...
            if self.SqOnOffFlag == 1:
                ...
                if matchFlag == 0:
                    ...
                    self.coord_on_canvas = self.canvas.create_rectangle(xpos - 20, ypix - ypos + 20, xpos + 20,
                                                                        ypix - ypos - 20, outline='#FF0000', width=5)
