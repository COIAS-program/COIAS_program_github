...
def calc_mean_detection_number(detect_thresh):
    ...
    default_sex_lines[15] = (
        "DETECT_THRESH    "
        + "{:.2f}".format(detect_thresh)
    ...
    default_sex_lines[9] = (
        ...
        + "/param/default2.param  # name of the file containing catalog contents\n"
    )
    default_sex_lines[19] = (
        "FILTER_NAME      "
        + PARAM.COIAS_DATA_PATH
        + "/param/default.conv   # name of the file containing the filter\n"
    )
    file.close()
...
try:
    ...
    print(
        ...
        + " detect_thresh="
        + "{:.2f}".format(detect_thresh)
        + "  mean_detection_number="
        + str(int(mean_detection_number))
    )
    print("----------------------------------------------------------------")
    # --------------------------------------------------
    ...
    if mean_detection_number > SOURCE_NUMBER * 1.25:
        ...
        if mean_detection_number < SOURCE_NUMBER * 0.75:
            ...
            while True:
                ...
                mean_detection_number_center = calc_mean_detection_number(
                    detect_thresh_center
                )
                ...
                print(
                    ...
                    + str(trial_number)
                    + " detect_thresh="
                    + "{:.2f}".format(detect_thresh_center)
                    + "  mean_detection_number="
                    + str(int(mean_detection_number_center))
                )
                print(
                    "----------------------------------------------------------------"
                )
                ...
                if (
                    mean_detection_number_center > SOURCE_NUMBER * 0.75
                    and mean_detection_number_center < SOURCE_NUMBER * 1.25
                ) or trial_number > 40:
                    break
        # -----------------------------------------------------------------
    ...
    print(
        "Some previous files are not found in findsource_auto_thresh_correct.py!",
        flush=True,
    )
