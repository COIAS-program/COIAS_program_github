...
try:
    if len(sys.argv)!=2:
        raise Exception("invalid argument. Please input the name of this script and return code of send_mail_to_MPC.py")
    ...
    if len(finalAllFileName)==0:
        raise FileNotFoundError("Any final_all.txt is not found in the current directory.")
    if len(finalAllFileName)>=2:
        print(f"something wrong. More than two final_all.txt files exist. files={finalAllFileName}")
    finalAllDestFileName = PARAM.COIAS_DATA_PATH + "/final_all_files/" + finalAllFileName[0]

    succeedSendMail = True if int(sys.argv[1])==0 else False
    if succeedSendMail:
        sendMpcDestFileName = PARAM.COIAS_DATA_PATH + "/send_mpc_files/succeeded_to_be_sent/" + sendMpcFileName[0]
    else:
        sendMpcDestFileName = PARAM.COIAS_DATA_PATH + "/send_mpc_files/failed_to_be_sent/" + sendMpcFileName[0]
    shutil.copyfile(sendMpcFileName[0], sendMpcDestFileName)
    shutil.copyfile(finalAllFileName[0], finalAllDestFileName)
    

except FileNotFoundError:
    print("Some previous files are not found in copy_sendMpc_and_finalAll_to_data_path.py!",flush=True)
