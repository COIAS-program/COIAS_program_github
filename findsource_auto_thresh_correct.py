...
def calc_mean_detection_number(detect_thresh):
    ...
    default_sex_lines[15] = "DETECT_THRESH    " + "{:.2f}".format(
        detect_thresh) + "          # <sigmas> or <threshold>,<ZP> in mag.arcsec-2\n"

    default_sex_lines[9] = "PARAMETERS_NAME  " + expanduser(
        "~") + "/.coias/default2.param  # name of the file containing catalog contents\n"
    default_sex_lines[19] = "FILTER_NAME      " + expanduser(
        "~") + "/.coias/default.conv   # name of the file containing the filter\n"
    file.close()
...
trial_number = 0
print("----------------------------------------------------------------")
print("#" + str(trial_number) + " detect_thresh=" + "{:.2f}".format(detect_thresh) + "  mean_detection_number=" + str(
    int(mean_detection_number)))
print("----------------------------------------------------------------")
# --------------------------------------------------
...
if mean_detection_number > 3000:
    while mean_detection_number > 3000:
        ...
        trial_number += 1
        print("----------------------------------------------------------------")
        print("#" + str(trial_number) + " detect_thresh=" + "{:.2f}".format(
            detect_thresh) + "  mean_detection_number=" + str(int(mean_detection_number)))
        print("----------------------------------------------------------------")
    ...
    if mean_detection_number < 2000:
        ...
        while True:
            ...
            print("----------------------------------------------------------------")
            print("#" + str(trial_number) + " detect_thresh=" + "{:.2f}".format(
                detect_thresh_center) + "  mean_detection_number=" + str(int(mean_detection_number_center)))
            print("----------------------------------------------------------------")
