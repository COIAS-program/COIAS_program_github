...
#ifdef WINDOWS_TEST_CODE
#include <dos.h>

static unsigned seg_to_descriptor( unsigned segment )
{
unsigned rval = 0;

_asm  mov   ax,002h           /* DPMI function: simulate real-mode int */
_asm  mov   bx,segment
_asm  int   31h               /* call DPMI */
_asm  jc    done
_asm  mov   rval,ax           /* return descriptor */
...
static void blast_mono( char *s, int line)
{
   char FAR *dispmem_ptr = NULL;

   _FP_SEG( dispmem_ptr) = seg_to_descriptor( 0xb000);
   dispmem_ptr += line * 160;
   while( *s)
      {
      *dispmem_ptr++ = *s++;
      *dispmem_ptr++ = 15;
