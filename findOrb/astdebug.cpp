...
#define LARGE_AXIS      ((unsigned long)(2100000000U))
#define VERY_LARGE_AXIS ((unsigned long)(3150000000U))
...
int DLL_FUNC setup_elems_from_ast_file( ELEMENTS DLLPTR *class_elem,
              const unsigned long DLLPTR *elem, const double t_epoch)
{
   double mean_anomaly;
...
static double kepler( const double ecc, double mean_anom)
{
   ...
   FILE *ofile = fopen( "ecc.txt", "ab");

   setvbuf( stdout, NULL, _IONBF, 0);

   fprintf( ofile, "%.15lf %.15lf %.15lf", ecc, mean_anom, curr);
