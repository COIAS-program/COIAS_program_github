...
#define POISSON_TERM struct poisson_term
...
int unpack_six_doubles( double *ovals, const char *ibuff)
{
   int i;
...
void * DLL_FUNC load_ps1996_series( FILE *ifile, double jd, int planet_no)
{
   ...
   fseek( ifile, (planet_no - 1) * sizeof( int32_t), SEEK_SET);
   fread( &offset, 1, sizeof( int32_t), ifile);
   fseek( ifile, offset, SEEK_SET);
   fread( &header, sizeof( POISSON_HEADER), 1, ifile);
   block = (int)floor( (jd - header.tzero) / header.dt);
   if( block < 0 || block >= header.n_blocks)
      {
      if( close_file)
         fclose( ifile);
   ...
   rval = (POISSON *)malloc( sizeof( POISSON) +
                                    header.total_fqs * 7 * sizeof( double));
   if( !rval)
      {
      if( close_file)
         fclose( ifile);
   ...
   fread( p.fqs, header.total_fqs, sizeof( double), ifile);

   fread( block_sizes, header.n_blocks, sizeof( int16_t), ifile);
   for( i = 0; i < block; i++)
      jump += (long) block_sizes[i];
   fseek( ifile, jump, SEEK_CUR);
   tbuff = (char *)malloc( block_sizes[block]);
   if( !tbuff)
      {
      free( rval);
      if( close_file)
   ...
   fread( tbuff, block_sizes[block], 1, ifile);
...
{
   FILE *ifile = fopen( "ps_1996.dat", "rb");
   double state_vect[6], t0 = atof( argv[2]), r;
   void *p;
