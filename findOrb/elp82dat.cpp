                 ...
                 const double *fund, const double prec, long n_terms)
{
   double rval = 0.;
   char *tptr, *ibuff;
   int coeffs[N_FUND_COEFFS];
   const int series_type = series_no / 3;
   int term_size, i, chunk_size;
   ...
   if( (long)chunk_size > n_terms)
      chunk_size = (unsigned)n_terms;
   ...
   while( n_terms--)
      ...
      if( ibuff == tptr)
         fread( ibuff, (size_t)chunk_size, term_size, ifile);
      ...
      tptr += term_size;
      if( tptr - ibuff == chunk_size * term_size)        /* end of line */
         tptr = ibuff;
      }
   free( ibuff);
   return( rval * 1.e-5);
                     ...
                     const double prec0, double *ovals)
{
   ...
   fread( hdr, sizeof( ELP_DATA_HEADER), 1, ifile);
                   ...
                   const double prec, double *ecliptic_xyz_2000)
{
   ...
   get_elp_values( ifile, t_cen, prec, uvr);
...
/* p = P_0 + t_cen * (P_1 + t_cen * (P_2 + t_cen * (P_3 + t_cen * P_4))); */
/* q = Q_0 + t_cen * (Q_1 + t_cen * (Q_2 + t_cen * (Q_3 + t_cen * Q_4))); */
   p *= t_cen;
   q *= t_cen;
