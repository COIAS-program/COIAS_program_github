                              ...
                              const int format_check_only)
{
   ...
   if( strlen( buff) > 200 && buff[10] == '.' && buff[16] == '.' &&
      ...
      if( buff[142] != ' ')         /* yes,  it is a perturbed orbit */
         {
         rval = 1;
...
int main( int argc, char **argv)
{
   ...
   if( argc == 2 && !memicmp( argv[1], "today", 5))
      {
   ...
   *buff = '\0';
   ...
   if( !memicmp( buff, " today", 6))
      dest_jd += atof( buff + 6);
   else
      dest_jd = get_time_from_string( dest_jd, buff, FULL_CTIME_YMD);
