...
try:
            ...
            for manualDeleteLine in manualDeleteList2Lines:
                for HConversionLine in HConversionListLines:
                    if manualDeleteLine.split()[0] == HConversionLine.split()[0]:
                        manualDeleteListLines.append(
                            HConversionLine.split()[1]
                            + " "
                            + manualDeleteLine.split()[2]
                            + "\n"
                        )
    ...
    print(
        "Some previous files are not found in correct_manual_delete_list.py!",
        flush=True,
    )
    ...
    print("Some errors occur in correct_manual_delete_list.py!", flush=True)
    print(traceback.format_exc(), flush=True)
    error = 1
    errorReason = 65
