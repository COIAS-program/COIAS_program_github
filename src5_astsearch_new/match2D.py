    ...
    if os.stat("listb2.txt").st_size == 0:
        ...
        for i in range(len(data1)):
            print_progress.print_progress(
                nCheckPointsForLoop=4, nForLoop=len(data1), currentForLoop=i
            )

            l = 0
            for j in range(len(data2[data1ImageNum[i]])):
                if (
                    data1[i, 2] - 0.0005 < data2[data1ImageNum[i]][j, 1]
                    and data1[i, 2] + 0.0005 > data2[data1ImageNum[i]][j, 1]
                    and data1[i, 3] - 0.0005 < data2[data1ImageNum[i]][j, 2]
                    and data1[i, 3] + 0.0005 > data2[data1ImageNum[i]][j, 2]
                ):
