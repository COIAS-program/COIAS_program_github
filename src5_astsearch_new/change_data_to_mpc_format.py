...
try:
    ...
    if os.stat("nomatch.txt").st_size == 0:
        ...
        for l in range(len(data)):
            ...
            mpc1Row = (
                ...
                + raDecStr
                + "         "
                + "{:.1f}".format(float(data[l][4])).rjust(4, "0")
                + " "
                + data[l][8]
                + "      "
                + "T09"
            ...
            fileUnknownDisp.write(
                nameStr + " " + data[l][9] + " " + data[l][6] + " " + data[l][7] + "\n"
            )
    ...
    if os.stat("match.txt").st_size == 0:
        ...
        for l in range(len(data)):
            ...
            if re.search(r"[a-zA-Z]", data[l][0]):
                nameStr = "     " + get_MPC_format_name_for_karifugo_asteroids(
                    data[l][0]
                )
                ...
                nameStr = (
                    get_MPC_format_name_for_numbered_asteroids(data[l][0]) + "       "
                )
    ...
    print(
        "Some previous files are not found in change_data_to_mpc_format.py!", flush=True
    )
    ...
    print("Some errors occur in change_data_to_mpc_format.py!", flush=True)
    print(traceback.format_exc(), flush=True)
    error = 1
    errorReason = 55
