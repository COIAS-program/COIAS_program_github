...
try:
    ...
    completed_process = subprocess.run(
        r"sed -e 's/\\//g' bright_asteroid_raw_names_in_the_field.txt > bright_asteroid_raw_names_in_the_field2.txt",
        shell=True,
    )
    ...
    completed_process = subprocess.run(
        "awk -F' ' '{if( ( ($1 ~ /^18[0-9][0-9]$/ || $1 ~ /^19[0-9][0-9]$/ || $1 ~ /^20[0-9][0-9]$/) && $2 ~ /^[A-Z][A-Z][0-9]*$/ ) || $2 == 'P-L' || $2 == 'T-1' || $2 == 'T-2' || $2 == 'T-3' ){print $1,$2}else{print $1}}' bright_asteroid_raw_names_in_the_field2.txt > bright_asteroid_raw_names_in_the_field3.txt",
        shell=True,
    )
    errorList.append(completed_process.returncode)
    ## remove space between karifugo
    completed_process = subprocess.run(
        "sed 's/ //g' bright_asteroid_raw_names_in_the_field3.txt > bright_asteroid_raw_names_in_the_field4.txt",
        shell=True,
    )
    errorList.append(completed_process.returncode)
    ...
    for l in range(len(lines)):
        if re.search(r"[a-zA-Z]", lines[l]):
            brightAsteroidsMPCNames.append(
                get_MPC_format_name_for_karifugo_asteroids(lines[l].rstrip("\n"))
            )
        else:
            brightAsteroidsMPCNames.append(
                get_MPC_format_name_for_numbered_asteroids(lines[l].rstrip("\n"))
            )

    ## output to each directory
    for n in range(Ndata):
        outputFile = open(
            directoryNames[n] + "/bright_asteroid_MPC_names_in_the_field.txt",
            "w",
            newline="\n",
        )
    ...
    print(
        "Some previous files are not found in make_asteroid_name_list_in_the_field.py!",
        flush=True,
    )
