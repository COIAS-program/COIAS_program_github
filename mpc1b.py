...
#temp = np.r_[name1,t8,rah2,ram2,decd3,decm2,decs3,mag2,fil[:,0],fil[:,1],data[:,4],data[:,5],data[:,6],data[:,7]]
    ...
    np.savetxt('unknown_mpc.txt',temp,fmt="%s")
    np.savetxt('unknown_all.txt',temp2,fmt="%s")
    np.savetxt('unknown_disp.txt',temp3,fmt="%s")
...
if os.stat("match.txt").st_size == 0:
    ...
    name = np.loadtxt("match.txt",dtype ='str',usecols=[0])
    data = np.loadtxt("match.txt",usecols=[1,2,3,4,5,6,7])
    fil = np.loadtxt("match.txt",dtype = 'str',usecols=[8,9])
...
#mag xy pixel
            ...
            temp2 = [name3,t8,rah2,ram2,decd3,decm2,decs3,mag2,filobs,fil[i,1],data[i,3],data[i,4],data[i,5],data[i,6]]
        ...
        np.savetxt("karifugo_mpc.txt",result1,fmt="%s")
        np.savetxt("karifugo_all.txt",result2,fmt="%s")
        np.savetxt("karifugo_disp.txt",result3,fmt="%s")
