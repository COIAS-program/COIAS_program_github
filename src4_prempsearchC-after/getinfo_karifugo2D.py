...
def getinfo(args):
    if args[3] == 1:
        print_progress.print_progress(
            nCheckPointsForLoop=19, nForLoop=args[5], currentForLoop=args[4]
        )
    ...
    try:
        objRadec = Horizons(
            id=args[0], location="568", epochs=args[1], id_type="smallbody"
        ).ephemerides()["targetname", "datetime_jd", "RA", "DEC", "V"]
    except ValueError:
        if args[3] == 1:
            pass
        if args[3] == 2:
...
try:
    if __name__ == "__main__":
        ...
        if haveAllPreciseOrbits == 0:
            ...
            if len(img_list) != Ndata:
                print(
                    "something wrong! len(img_list)={0:d} Ndata={1:d}".format(
                        len(img_list), Ndata
                    )
                )
                raise Exception
            ...
            # time_list
            time_list2 = [
                np.round(float(time_list[i]), decimals=8) for i in range(len(time_list))
            ]
            ...
            args = [
                (name_list[i], time_list2, isCorrectDirectory, 1, i, len(name_list))
            ...
            secondArgs = [
                (second_name_list[i], time_list2, isCorrectDirectory, 2)
            ...
            if len(tmp10)==0:
                tmp5 = np.zeros(len(tmp10),dtype=object)
            else:
            ...
            for l in range(len(tmp8)):
                tmp8[l, 0] = (
                    tmp8[l, 0].replace("(", "").replace(")", "").replace(" ", "")
                )
            ...
            for i in range(Ndata):
                if isCorrectDirectory[i] == 0:
                    fList[i] = open(
                        directoryNames[i] + "/karifugo_new2B.txt", "w", newline="\n"
                    )

            for i in range(len(tmp8)):
                if isCorrectDirectory[int(tmp8[i, 5])] == 0:
                    fList[int(tmp8[i, 5])].write(
                        tmp8[i, 0]
                        + " "
                        + str(tmp8[i, 1])
                        + " "
            ...
            for i in range(Ndata):
                if isCorrectDirectory[i] == 0:
                    raDecJdTimeFile = open(
                        directoryNames[i] + "/ra_dec_jd_time.txt", "w", newline="\n"
                    )
                    raDecJdTimeFile.write(
                        "{0:.5f} {1:.5f} {2:.7f} {3:.1f}".format(
                            ra_list[i], dec_list[i], time_list[i], presentTimeStamp
                        )
                    )
    ...
    print("You do not connect to the internet in getinfo_karifugo2D.py!", flush=True)
    print(traceback.format_exc(), flush=True)
    error = 1
    errorReason = 42

except (requests.exceptions.ConnectTimeout, requests.exceptions.ReadTimeout):
    print("Connection timeout to NASA JPL in getinfo_karifugo2D!", flush=True)
    print(traceback.format_exc(), flush=True)
    error = 1
    errorReason = 43

except FileNotFoundError:
    print("Some previous files are not found in getinfo_karifugo2D.py!", flush=True)
    print(traceback.format_exc(), flush=True)
    error = 1
    errorReason = 44

except Exception:
    print("Some errors occur in getinfo_karifugo2D.py!", flush=True)
    print(traceback.format_exc(), flush=True)
    error = 1
    errorReason = 45
