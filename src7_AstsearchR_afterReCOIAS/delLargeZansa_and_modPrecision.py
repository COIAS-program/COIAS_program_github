#!/usr/bin/env python3
# -*- coding: UTF-8 -*
# Timestamp: 2023/08/29 20:30 sugiura
###########################################################################################
# findorb.pyによって計算された残差が0.7"より大きいデータ点を削除する.
# (2023/8/29)さらにmagが26.5等級よりも暗い天体は誤測定である可能性が高いので, それらも削除する.
...
# 出力: pre_repo.txt
# 　　    残差が0.7"よりも大きいもしくはmagが26.5等級よりも大きいデータを削除し,
#        さらに天体あたりのデータ行数が2以下になったものを削除し,
...
try:
    ...
    if os.stat(inputFileName).st_size != 0:
        ...
        for i in range(len(inputLines)):
            ...
            mag = float(contents[10])

            if (abs(Xres) < 0.7) and (abs(Yres) < 0.7) and (mag < 26.5):
