...
try:
    ...
    if (not os.path.isfile("manual_name_modify_list.txt")) or (
        ...
        completed_process = subprocess.run(
            "cp newall_automanual.txt newall_automanual2.txt", shell=True
        )
        if completed_process.returncode != 0:
            raise FileNotFoundError
        completed_process = subprocess.run(
            "cp redisp_automanual.txt redisp_automanual2.txt", shell=True
        )
        if completed_process.returncode != 0:
            raise FileNotFoundError
        ...
        for l in range(len(beforeNameList)):
            if line.split()[0] == beforeNameList[l]:
                ...
                if len(afterNameList[l]) == 5:
                    ...
                    fileMpc4Automanual2.write(
                        line.replace(beforeNameList[l], afterNameList[l])
                    )
        if not isMatch:
            fileMpc4Automanual2.write(line)
    fileMpc4Automanual2.close()
    completed_process = subprocess.run(
        "sort -n -o mpc4_automanual2.txt mpc4_automanual2.txt", shell=True
    )
    if completed_process.returncode != 0:
        raise Exception
    ...
    for line in lines:
        ...
        for l in range(len(beforeNameList)):
            if line.split()[0] == beforeNameList[l]:
                isMatch = True
                if len(afterNameList[l]) == 5:
                    fileNewallAutomanual2.write(
                        afterNameList[l] + "       " + line[12:]
                    )
                else:
                    fileNewallAutomanual2.write(
                        line.replace(beforeNameList[l], afterNameList[l])
    ...
    completed_process = subprocess.run(
        "sort -n -o newall_automanual2.txt newall_automanual2.txt", shell=True
    )
    if completed_process.returncode != 0:
        raise Exception
    ...
    completed_process = subprocess.run(
        "sort -k 1,1 -k 2n,2 -o redisp_automanual2.txt redisp_automanual2.txt",
        shell=True,
    )
    if completed_process.returncode != 0:
        raise Exception
    ...
    print(
        "Some previous files are not found in aplly_manual_name_modify.py!", flush=True
    )
    ...
    print("Some errors occur in aplly_manual_name_modify.py!", flush=True)
    print(traceback.format_exc(), flush=True)
    error = 1
    errorReason = 75
